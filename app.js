...
// Mock components for demonstration purposes if shadcn/ui is not directly available in the sandbox.
// In a real shadcn/ui setup, these imports would resolve to actual components.
  ...
  return (
    <div>
      {React.Children.map(children, child => {
        if (React.isValidElement(child)) { // Ensure child is a valid React element
...
const TabsList = ({ children, activeTab, setActiveTab }) => (
  <div className="inline-flex h-9 items-center justify-center rounded-lg bg-muted p-1 text-muted-foreground mb-4">
    {React.Children.map(children, child => {
      // Ensure child is a valid React element before accessing its props
      ...
      return null; // Ignore non-element children (like whitespace, comments)
    })}
  </div>
);
    ...
    <div className="p-6 space-y-6 bg-gradient-to-br from-blue-50 to-white min-h-screen font-sans">
      ...
      */}
      ...
      <style>{`
        ...
        pre {
          overflow-x: auto; /* Ensure horizontal scrolling for code blocks on smaller screens */
          white-space: pre-wrap;
          word-wrap: break-word; /* Break long words */
        }
        /* Consistent padding for table headers and data cells */
        table th, table td {
          padding: 0.75rem;
        }
        /* Ensure lists have consistent indentation and spacing */
        ul.list-disc, ol.list-decimal {
          margin-left: 1.5rem; /* Standard indent for lists */
        }
        ul.list-disc li, ol.list-decimal li {
            margin-bottom: 0.5rem; /* Space between list items */
        }
        /* Remove extra left margin from paragraphs inside list items if parent ul/ol provides it */
      ...
      <section className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto">
        ...
        {/* BEGINNER'S GUIDE CARD - Moved to the top */}
        <Card className="bg-white shadow-xl rounded-xl lg:col-span-3"> {/* Full width */}
